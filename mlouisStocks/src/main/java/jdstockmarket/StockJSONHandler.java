...
public class StockJSONHandler implements JSONHandler {
	...
	// Named constants
	private static final String TIME_SERIES_KEY_5MIN = "Time Series (5min)";
	private static final String TIME_SERIES_KEY_30MIN = "Time Series (30min)";
    ...
    public JSONObject fetchStockData(StockMarketAPI stockAPI, String stockSymbol, Interval interval ) throws IOException {
        String stockData = stockAPI.fetchLiveStockData(stockSymbol, interval);
    ...
    public String displayStockInfo(JSONObject stockJSON, String stockSymbol) {
        ...
        if (stockJSON.has("Time Series (5min)")) {
            JSONObject timeSeries = getValue(stockJSON, TIME_SERIES_KEY_5MIN);
